...
const Navbar = () => {
    ...
    const {search, showSearch, setShowSearch, setSearch, getCartCount, navigate, token, setToken} = useContext(ShopContext);
      ...
      <div className='flex items-center gap-6'>
            ...
            <div className='group relative'>
                ...
                <div className="group-hover:block hidden absolute dropdown-menu right-0 pt-4">
                    <div className='flex flex-col gap-2 w-36 py-3 px-5 bg-slate-100 text-gray-700 rounded'>
                        ...
                        <p onClick={() =>window.open(import.meta.env.VITE_ADMIN_URL, '_blank')} className='cursor-pointer hover:text-black' >Admin</p>
        ...
        <div className={`absolute top-0 right-0 bottom-0 bg-white overflow-hidden transition-all ${visible ? 'w-full': 'w-0'}`}>
            <div className="flex flex-col text-gray-600">
                ...
                <NavLink onClick={() => setVisible(false)} className='py-2 pl-6 border cursor-pointer' to={import.meta.env.VITE_ADMIN_URL}>ADMIN</NavLink>
            </div>
        </div>
    </div>
  )
